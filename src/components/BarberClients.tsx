...
const BarberClients: React.FC = () => {
  ...
  const fetchDetails = (client: Client) => {
    setSelected(client);
    setNotes(client.notes || '');
    setLoading(true);
    fetch(`/api/barbers/clients/${client.id}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then(res => res.json())
      .then(data => {
  ...
  const handleSaveNotes = async () => {
    ...
    setNotesSuccess('');
    try {
      const res = await fetch(`/api/barbers/clients/${selected?.id}/notes`, {
        method: 'PUT',
        headers: {
