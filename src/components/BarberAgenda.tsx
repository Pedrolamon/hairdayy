...
import { Calendar, Trash2, Edit, Save, X, Plus, Clock, Users, Tag, Check, Filter, Bell, AlertCircle } from 'lucide-react';

// Tipagem para o Enum de Status de Agendamento, definido localmente no frontend.
...
const BarberAgenda: React.FC = () => {
  ...
  // Efeito para logar o estado do novo formulário, para depuração.
  useEffect(() => {
  ...
  const fetchAppointments = useCallback(async () => {
    ...
    setError('');
    try {
      // Removido o unitId da URL
      const res = await fetch(`/api/appointments?barberId=${user.id}&date=${filterDate}`, {
  ...
  const fetchAvailableSlots = useCallback(async () => {
    ...
    if (!date || !serviceId || !user || !token) {
      setAvailableSlots([]);
      return;
    }
    try {
      const res = await fetch(`/api/appointments/available?serviceId=${serviceId}&barberId=${user.id}&date=${date}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDelete = async () => {
    if (!appointmentToDelete || !token) return;
  ...
  const handleNewFormSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Valores do formulário:', newForm);
      console.log('Verificação de variáveis de contexto na submissão:', {
        tokenExists: !!token,
        userExists: !!user,
        // Removida a verificação de unitExists
    });
    // Removida a verificação de unit
    if (!token || !user || !newForm.date || !newForm.serviceId || !newForm.startTime) {
  ...
  const handleEditFormSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!token || !selectedAppointment || !editForm.date || !editForm.startTime || !editForm.endTime) return;
